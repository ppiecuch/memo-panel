...
static inline void rotate_area_cw(const lv_color_t *in, uint16_t *out, int32_t in_height, int32_t in_line_length, int32_t out_line_length, int32_t area_x1, int32_t area_y1, int32_t area_x2, int32_t area_y2);
...
void fbdev_init(void) {
	...
	printf("%dx%d, line length:%d, %dbpp, rotate:%d\n", vinfo.xres, vinfo.yres, vinfo.bits_per_pixel, finfo.line_length, vinfo.rotate);
...
void fbdev_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p) {
	...
	/*16 bit per pixel*/
		...
		if (1) {
			rotate_area_cw(color_p, fbp16, vinfo.xres, vinfo.yres, finfo.line_length, act_x1, act_y1, act_x2, act_y2);
		} else {
			int32_t y;
			for (y = act_y1; y <= act_y2; y++) {
